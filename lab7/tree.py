...
=== Module Description ===
This module contains starter code for Lab 7.
...
class Tree:
    ...
    def __init__(self, root: Optional[Any], subtrees: List[Tree], size: int) -> None:
    ...
    def branching_factor(self) -> float:
        ...
        3.0
        """
        # 9 / 3  -> number of internal nodes / number of sub trees
        if self.is_empty() or self._subtrees == []:
            return 0.0

        sum_trees = self.avghelper()
        return sum(sum_trees)/len(sum_trees)

    def avghelper(self) -> List:
        if self.is_empty() or self._subtrees == []:
            return []

        # a list of number of subtrees
        l = [len(self._subtrees)]
        for i in self._subtrees:
            l.extend(i.avghelper())
    ...
    def items_at_depth(self, d: int) -> List:
        ...
        for sub in self._subtrees:
            items.extend(sub.items_at_depth(d - 1))
        return items
    ...
    def insert(self, item: Any) -> None:
        ...
        elif not self._subtrees or randnum == 3:
            self._subtrees.append(Tree(item, [], 1))

        else:
            random_subtree = random.choice(self._subtrees)
            random_subtree.insert(item)
