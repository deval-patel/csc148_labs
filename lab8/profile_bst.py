...
def insert_delete_all(n: int, sorted_: bool) -> None:
    ...
    If <sorted_> is True, the items should be processed in sorted order.
    Otherwise, the items should be processed in *random* order.
...
if __name__ == '__main__':
    ...
    for size in SIZES:
        ...
        sorted_items.append(time)

    plt.figure()
    plt.plot(SIZES, random_items)
    plt.xlabel('BST Sizes')
    plt.ylabel('Time')
    plt.suptitle('Random Insert times')
    plt.show()
    plt.figure()
    plt.plot(SIZES, sorted_items)
    plt.xlabel('BST Sizes')
    plt.ylabel('Time')
    plt.suptitle('Sorted Insert times')
    plt.show()
